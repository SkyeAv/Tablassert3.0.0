...
def test_namespace_uuid_returns_uuid() -> None:
    ...
    result: str = tablassert_rs.namespace_uuid("domain", ["a", "b"])
...
def test_dedup_ndjson_deduplicates_nodes(tmp_path: Path) -> None:
    ...
    tablassert_rs.dedup_ndjson(p_in, p_out, False, "TABLASSERT")
...
def test_dedup_ndjson_labels_edges(tmp_path: Path) -> None:
    ...
    tablassert_rs.dedup_ndjson(p_in, p_out, True, "TABLASSERT")
