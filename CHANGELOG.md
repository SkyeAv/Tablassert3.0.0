...
### Changed
- **The fullmap database is now hash-sharded on disk.** The single redb file is split into a small primary file (`fullmap.redb` — dimensions, `curies`, and `meta`) plus four RECORDS shard files (`fullmap.s0.redb` … `fullmap.s3.redb`), with each normalized term routed to a shard by `xxh64(term) & 3`. The on-disk schema is now `tablassert.fullmap.v4`; existing `v3` databases are rejected at lookup time and must be rebuilt with `tablassert build-fullmap`. Two phases were parallelized as a result: the Phase-2 RECORDS write now runs a single k-way merge feeding bounded per-shard channels drained by `min(shard_count, worker_count)` concurrent redb writers (one database per shard — redb is single-writer per file), lifting the B-tree-insert bottleneck (~N× write throughput); and fullmap lookup now releases the GIL and fans the query terms out across the shards in parallel (one reader per non-empty shard, re-merged into input order). The shard files are discovered as siblings of the primary path and must remain alongside it; see `docs/fullmap.md`.
