...
### New Features
- Added `tablassert-cli verify-table-configuration-syntax -i <table-config.yaml>` for fast TC3 schema validation without running a full graph build.
...
### Changes
- Updated the CLI interface for graph builds from `tablassert-cli -i <graph-config.yaml>` to `tablassert-cli build-knowledge-graph -i <graph-config.yaml>`.
